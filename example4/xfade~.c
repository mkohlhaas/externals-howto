// source-code for the fourth example in the HOWTO
...
{
  // post("Entering XFADE_TILDE_PERFORM");
  t_xfade_tilde *x = (t_xfade_tilde *)(w[1]);
  t_sample    *in1 =      (t_sample *)(w[2]);
  t_sample    *in2 =      (t_sample *)(w[3]);
  t_sample    *out =      (t_sample *)(w[4]);
  int            n =             (int)(w[5]);
  t_sample pan     = (x->x_pan<0) ? 0.0 : 
                     (x->x_pan>1) ? 1.0 : x->x_pan;
...
{
  post("Entering xfade_tilde_dsp");
  // add xfade_tilde_perform() to the DSP-tree;
  // xfade_tilde_perform() will expect "5" arguments (packed into a t_int-array), which are:
  // the objects data-space, 3 signal vectors (which happen to be 2 input signals and 1 output signal)
...
void xfade_tilde_setup(void) {
  ...
  // whenever the audio-engine is turned on, the "xfade_tilde_dsp()" function will get called
  class_addmethod(xfade_tilde_class, xfade_tilde_dsp, gensym("dsp"), A_CANT, 0);
  // class will use signal-inlets
  // if no signal is connected to the first inlet, we can as well connect a number box to it and use it as "signal"
